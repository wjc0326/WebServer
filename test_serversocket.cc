...
void TestSSThreadFn(ThreadPool::Task *t) {
  ...
  ASSERT_TRUE(ss.bind_and_listen(&listen_fd));

  // Accept a connection.
  cout << "Doing accept..." << endl;
  ASSERT_TRUE(ss.accept_client(&task->accept_fd, &task->caddr, &task->cport,
...
TEST(Test_ServerSocket, Basic) {
  ...
  cout << "Attempting to connect to 127.0.0.1 port "
       << portnum << endl;
  int cfd = -1;
  ASSERT_TRUE(connect_to_server("127.0.0.1", portnum, &cfd));
  ...
  // record points
  ProjectEnvironment::AddPoints(25);
